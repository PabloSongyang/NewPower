...
public class Buy_list_funtion : MonoBehaviour {
    ...
    IEnumerator delate()
    {
        ...
        yield return new WaitForSeconds(0.1f);
        if (father.transform.parent.GetComponent<ContentSizeFitter>())
        {
            Debug.Log(21312321);
            father.transform.parent.GetComponent<ContentSizeFitter>().enabled = false;
            yield return new WaitForSeconds(0.1f);
            father.transform.parent.GetComponent<ContentSizeFitter>().enabled = true;
        }
    }
}
